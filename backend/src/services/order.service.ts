...
export const OrderService = {
  async listForCustomer(userId: string, jwt: string) {
    ...
    const { data, error } = await db.from('orders').select('*').eq('customer_id', userId).order('placed_at', { ascending: false });
  ...
  async listForVendor(userId: string, jwt: string) {
    ...
    const { data, error } = await db.from('orders').select('*').eq('vendor_id', vendor!.id).order('placed_at', { ascending: false });
    if (error) throw new BadRequestError(error.message);
    return data;
  },
