...
export const ChatService = {
  ...
  async listConversations(userId: string, jwt: string) {
    const db = supabaseClient('anon', jwt);
    const { data, error } = await db.from('conversations').select('*').eq('customer_id', userId).order('created_at', { ascending: false });
    if (error) throw new BadRequestError(error.message);
    return data;
  },
  ...
  async sendMessage(userId: string, jwt: string, conversationId: string, content: string) {
    const db = supabaseClient('anon', jwt);
    const { data, error } = await db.from('messages').insert({ conversation_id: conversationId, sender_id: userId, content }).select('*').single();
    if (error) throw new BadRequestError(error.message);
    return data;
  },
