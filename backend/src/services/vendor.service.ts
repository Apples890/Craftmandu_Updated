...
export const VendorService = {
  async getBySlug(slug: string) {
    ...
    const { data, error } = await db.from('vendors').select('*').eq('slug', slug).single();
  ...
  async updateMine(userId: string, jwt: string, patch: Partial<VendorDbRow>) {
    ...
    const { data, error } = await db.from('vendors').update(allowed).eq('user_id', userId).select('*').single();
